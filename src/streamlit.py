...
from src.utils import wp_utils  # Add WordPress service import
...
# Add WordPress to the sidebar menu
if 'page' not in st.session_state:
    st.session_state['page'] = 'home'
if 'wordpress_page' not in st.session_state:
    st.session_state['wordpress_page'] = 1
if 'wordpress_search' not in st.session_state:
    st.session_state['wordpress_search'] = ''
...
# Add WordPress menu
menu_options = ['Home', 'WordPress']
selected_menu = st.sidebar.radio(tr("Menu"), menu_options)

if selected_menu == 'Home':
    st.session_state['page'] = 'home'
else:
    st.session_state['page'] = 'wordpress'
...
if st.session_state['page'] == 'wordpress':
    with st.sidebar.expander(tr("WordPress Settings"), expanded=False):
        ...
        wp_password = st.text_input(tr("Password (optional)"), value=saved_wp_password, type="password")
        ...
        if st.button(tr("Test Connection")):
            try:
                if not wp_url:
                    st.error(tr("WordPress URL is required"))
                else:
                    wp_client = wp_utils.WordPress(wp_url, wp_username, wp_password)
                    result = wp_client.get_posts(page=1, per_page=1)
                    if 'error' in result:
                        st.error(result['error'])
                    else:
                        st.success(tr("Connection successful!"))
            except Exception as e:
                st.error(f"Connection failed: {str(e)}")

# Check which page to display
if st.session_state['page'] == 'home':
    # Display the original UI with video generation functionality
    panel = st.columns(3)
    left_panel = panel[0]
    ...
    if start_button:
        ...
        result = tm.start(params, StopAt.VIDEO)
...
elif st.session_state['page'] == 'wordpress':
    ...
    if not wp_url:
        ...
        with col1:
            search_query = st.text_input(
                tr("Search Posts"), 
                value=st.session_state['wordpress_search'],
                placeholder=tr("Enter keywords to search")
            )
        with col2:
            if st.button(tr("Search"), use_container_width=True):
                st.session_state['wordpress_search'] = search_query
                st.session_state['wordpress_page'] = 1
                st.session_state['selected_post_id'] = None
        
        # Initialize WordPress client
        try:
            wp_client = wp_utils.WordPress()  # No need to pass parameters as they're in config now
            
            # Display selected post or posts list
            if st.session_state['selected_post_id']:
                # Back to posts list button
                if st.button(tr("← Back to Posts List")):
                    st.session_state['selected_post_id'] = None
                    st.rerun()
                
                # Fetch and display post details
                post = wp_client.get_post(st.session_state['selected_post_id'])
                ...
                if 'error' in post:
                    ...
                    st.markdown(f"*{tr('Published')}: {post['date']}*")
                    
                    # Featured image if available
                    if post['featured_image']:
                        st.image(post['featured_image'], use_column_width=True)
                    ...
                    if post['author']:
                        st.subheader(tr("Author"))
                        author_col1, author_col2 = st.columns([1, 4])
                        with author_col1:
                            if post['author']['avatar']:
                                st.image(post['author']['avatar'], width=60)
                        with author_col2:
                            st.markdown(f"**{post['author']['name']}**")
                    
                    # Display categories and tags
                    if post['categories']:
                        st.subheader(tr("Categories"))
                        st.markdown(", ".join([cat['name'] for cat in post['categories']]))
                    
                    if post['tags']:
                        st.subheader(tr("Tags"))
                        st.markdown(", ".join([tag['name'] for tag in post['tags']]))
            else:
                # Display posts list with pagination
                result = wp_client.get_posts(
                    page=st.session_state['wordpress_page'], 
                    per_page=10, 
                    search=st.session_state['wordpress_search']
                )
                ...
                if 'error' in result:
                    ...
                    # Display posts
                    if not result['posts']:
                        st.info(tr("No posts found"))
                    else:
                        st.subheader(f"{tr('Found')} {result['total_posts']} {tr('posts')}")
                        
                        # Display each post
                        for post in result['posts']:
                            with st.container(border=True):
                                post_cols = st.columns([3, 1])
                                with post_cols[0]:
                                    st.markdown(f"### {post['title']}")
                                    st.markdown(post['excerpt'], unsafe_allow_html=True)
                                    st.markdown(f"*{post['date']}*")
                                    
                                    # View post button
                                    if st.button(tr("View Post"), key=f"view_{post['id']}"):
                        ...
                        if result['total_pages'] > 1:
                            pagination_cols = st.columns(min(result['total_pages'] + 2, 11))
                            
                            # Previous button
                            with pagination_cols[0]:
                                if st.session_state['wordpress_page'] > 1:
                                    if st.button("←", use_container_width=True):
                                        st.session_state['wordpress_page'] -= 1
                                        st.rerun()
                            
                            # Page buttons (show up to 9 page buttons)
                            start_page = max(1, min(
                                st.session_state['wordpress_page'] - 4,
                                result['total_pages'] - 8
                            ))
                            end_page = min(result['total_pages'], start_page + 8)
                            
                            for i, page_num in enumerate(range(start_page, end_page + 1)):
                                with pagination_cols[i + 1]:
                                    button_type = "primary" if page_num == st.session_state['wordpress_page'] else "secondary"
                                    if st.button(f"{page_num}", key=f"page_{page_num}", use_container_width=True, type=button_type):
                                        st.session_state['wordpress_page'] = page_num
                                        st.rerun()
                            
                            # Next button
                            with pagination_cols[-1]:
                                if st.session_state['wordpress_page'] < result['total_pages']:
                                    if st.button("→", use_container_width=True):
                                        st.session_state['wordpress_page'] += 1
                                        st.rerun()
